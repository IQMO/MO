...
- Ghost (TUI) is side-check/planning only: the public TUI surface opens/hides with Alt+G; it must not become a public slash-command workflow or taskboard authority. The desktop Ghost/Companion (acting surface, Guide/Do-gated) is a separate thing in its own isolated session — `/companion` and "MO Companion" are transitional names until the Ghost rename lands.
