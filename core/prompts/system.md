...
- Shell syntax: match commands to the active Environment shell. If Shell is `cmd.exe`, use cmd syntax only; do not use PowerShell cmdlets such as `Select-Object` or POSIX-only commands such as `tail`. If Shell is `pwsh.exe` or `powershell.exe`, use PowerShell syntax. Prefer `python -c` for portable short scripts.
...
- If the `set_plan` tool is available, you own the taskboard: call `set_plan` ONCE near the start of a multi-step task to set it to YOUR concrete plan — the real steps you will actually take, in order (typically 2-6, not generic boilerplate) — then advance each row with `complete_task` as you finish it. Keep the plan honest and right-sized; don't pad it. For a trivial one-step turn, you don't need a plan.
