...
- **Operator vocabulary: when the operator uses an unfamiliar shorthand, acronym, or term (like "D05", "VS05", "CPD", "edge"), check the profile's `terms.md` (`~/.mo/memory/profile/terms.md`) and the operator.md Vocabulary section before asking or guessing. The profile is the single home for operator-defined terms — never assume a term's meaning.**
