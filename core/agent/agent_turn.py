...
class AgentTurn(AgentTurnDispatchMixin, AgentTurnRecoveryMixin):
    ...
    def _build_extra_context(self, user_input: str) -> str:
        ...
        and reasoning level preference.  Shared by run_turn and run_turn_streaming.
    ...
    def _call_provider_stream(self, extra_context: str | None = None):
        """Stream from the active provider, yielding raw chunks."""
        messages = self.session.get_messages(extra_context=extra_context, consume_handoff=False)
        handoff_seed = str(getattr(self.session, "_handoff_context", "") or "")
        p = self.active_provider
        stream = p.stream(
        ...
        def _consume_stream():
            ...
            try:
                ...
                if completed and handoff_seed and str(getattr(self.session, "_handoff_context", "") or "") == handoff_seed:
                    self.session._handoff_context = ""
    ...
    def _empty_response_action(self, *, empty_response_prompts: int, empty_response_fallback_attempted: bool,
        ...
        Same decision in run_turn and run_turn_streaming — keep it here so a fix
        cannot land in one mode and miss the other.
        """
    ...
    def run_turn_streaming(self, user_input: str, task_board: TaskBoard | None = None, monitor: BackendMonitor | None = None, on_board_update: object = None, on_first_tool: object = None, on_board_event: object = None):
        """Execute a turn with streaming events yielded for real-time display.

        Yields dicts:
          {"type": "text", "content": "..."}           — text delta from model
          {"type": "reasoning", "content": "..."}       — reasoning delta (DeepSeek)
          {"type": "tool_start", "name": "...", "id": "..."} — about to dispatch a tool
          {"type": "tool_end", "name": "...", "result": "...", "id": "..."} — tool dispatched
          {"type": "task_update", "board": "..."}       — task board changed (rich markup)
          {"type": "done", "final_text": "..."}          — turn complete
          {"type": "error", "message": "..."}            — error occurred
        """
        start = self._prepare_turn_start(user_input, monitor=monitor, streaming=True)
        user_input = str(start.get("user_input") or "")
        if start.get("final_text") is not None:
            yield {"type": "done", "final_text": str(start.get("final_text") or "")}
            return

        extra_context = self._build_extra_context(user_input)
        handoff_started = self._maybe_context_handoff(user_input, extra_context=extra_context)
        if handoff_started:
        ...
        self._emit_sanitize_event(monitor, sanitize_meta, stage="pre_provider_loop_streaming")

        # === Phase 2: Provider request loop (streaming) ===
        while provider_requests < self.max_provider_requests or (provider_limit_grace and provider_requests == self.max_provider_requests):
            live_steer_context = self._consume_live_steer_context(monitor=monitor)
            if live_steer_context:
                extra_context = (extra_context or "") + "\n\n" + live_steer_context
                yield {"type": "status", "message": "Applying operator steer..."}
            provider_requests += 1
            if provider_requests >= self.max_provider_requests and not provider_limit_grace:
                provider_limit_grace = True
                self._grant_provider_request_grace(
            ...
            # === Phase 2a (streaming): stream response chunks and accumulate ===
            # Stream the provider response
            content_parts: list[str] = []
            reasoning_parts: list[str] = []
            tool_call_acc: dict[str, dict] = {}  # index -> {id, name, arguments}
            usage = None
            finish_reason = ""

            # Pre-call capacity gate (streaming): skip exhausted providers proactively
            cap = get_capacity()
            if not cap.can_accept(self.provider_name):
                cap_reason = "primary provider rate/concurrency limit (pre-call capacity check)"
                if self._next_provider(cap_reason):
            ...
            try:
                stream = self._call_provider_stream(extra_context=extra_context)
            except Exception as exc:
                # === Phase 2a error (streaming): classify, recover, or fallback ===
                raw_error = str(exc)
                if is_rate_limit_error(raw_error) or fallback_reason(raw_error):
                    try:
                        get_capacity().record_error(self.provider_name, raw_error)
                ...
                if is_context_overflow and not context_overflow_retry_attempted:
                    ...
                    if self._recover_from_provider_context_overflow(
                        ...
                        yield {"type": "status", "message": "context recovered, retrying provider request"}
                        continue
                if reason and reason != "provider_context_overflow" and self._next_provider(reason):
                    if monitor:
                        monitor.emit("provider_fallback", {"request": provider_requests, "provider": self.provider_name, "model": self.model, "reason": reason})
                    turn_provider_fallbacks += 1
                    continue
                yield {"type": "error", "message": f"MO provider error: {err_msg}"}
                return

            try:
                for chunk in stream:
                    # Extract delta from chunk (OpenAI format)
                    choices = getattr(chunk, "choices", None) or []
                    if not choices:
                        # Codex Responses API format
                        etype = getattr(chunk, "type", None)
                        if etype in {"response.failed", "response.incomplete"}:
                            detail = getattr(chunk, "error", None) or getattr(chunk, "response", None) or chunk
                            raise RuntimeError(f"Stream failed: {detail}")
                        # Handle Codex response events
                        if etype == "response.output_text.delta":
                            delta_text = getattr(chunk, "delta", "") or ""
                            if delta_text:
                    ...
                    if delta_content:
                        if task_board:
                            report_id = self._final_report_task_id(task_board)
                            if report_id and task_board.active_task_id() != report_id and task_board.activate(report_id):
                                record_snapshot(task_board, "updated")
                                rich = _emit_task_board_update(task_board, update="updated", on_board_update=on_board_update, on_board_event=on_board_event)
                                yield {"type": "task_update", "board": rich}
                    ...
                    if not delta_reasoning:
                        # Some providers use different field
                        delta_reasoning = (delta.get("reasoning_content") if isinstance(delta, dict) else "") or ""
                    if delta_reasoning:
                        reasoning_parts.append(delta_reasoning)
                        yield {"type": "reasoning", "content": delta_reasoning}
                    ...
                    for tc in tool_deltas:
                        ...
                        if fn_name:
                            tool_call_acc[idx]["name"] = fn_name
                        if fn_args:
                            tool_call_acc[idx]["arguments"] += fn_args
                ...
                if monitor:
                    monitor.emit("provider_error", {"request": provider_requests, "provider": self.provider_name, "reason": reason or "error", "error": err_msg[:300]})
                has_partial_stream = bool(content_parts or reasoning_parts or tool_call_acc)
                if is_context_overflow and not has_partial_stream and not context_overflow_retry_attempted:
                    context_overflow_retry_attempted = True
                    if self._recover_from_provider_context_overflow(
                        latest_user=user_input,
                        extra_context=extra_context,
            ...
            if monitor:
                monitor.emit("provider_response", {
                    ...
                    "provider": self.provider_name,
                    "finish_reason": finish_reason,
                    "tool_calls": len(tool_call_acc),
                    "content_chars": len("".join(content_parts)),
                    "preview": preview_provider_response("".join(content_parts), []),
                })

            # Collect tool calls
            tool_calls_list = [
                tc for tc in tool_call_acc.values()
                if tc.get("name")
            ]

            # === Phase 2c (streaming): dispatch tool calls ===
            if tool_calls_list:
                if self._devmode05_completed_taskboard_should_stop_tools(user_input, task_board):
                    self._devmode05_completed_board_tool_blocked_count += 1
                    if monitor:
                    ...
                    if self._devmode05_completed_board_tool_blocked_count <= 2:
                        self.session.add_assistant(self._devmode05_completed_taskboard_tool_instruction())
                        yield {"type": "status", "message": "DEVMODE05: taskboard complete — closeout required without more tools..."}
                        continue
                    yield {"type": "error", "message": self._devmode05_completed_taskboard_persistent_tool_text()}
                    return
                # Adaptive turn management: block tools after handoff budget exhausted
                if getattr(self, '_turn_health_tools_blocked', False):
                    self._turn_health_tools_blocked_count += 1
                    if self._turn_health_tools_blocked_count <= 2:
                        ...
                        continue
                    # After 2 blocked attempts, hard stop
                    yield {"type": "error", "message": self._turn_health_persistent_block_text(user_input)}
                    return
                tool_rounds += 1
                if tool_rounds > self.max_tool_rounds:
                    action, tool_limit_grace_used = self._handle_tool_round_limit(
                    ...
                    if action == "grace":
                        continue
                    message = self._finalize_limit_exhaustion(
                        kind="max_tool_rounds",
                        limit=self.max_tool_rounds,
                        tool_rounds=tool_rounds,
                        provider_requests=provider_requests,
                ...
                for tc in tool_calls_list:
                    tc_id = tc["id"] or f"call_{abs(hash(tc['name'] + tc['arguments'])) % 10_000_000}"
                    tool_calls_data.append({
                        "id": tc_id,
                        "type": "function",
                ...
                for tc_data in tool_calls_data:
                    name = tc_data["function"]["name"]
                    arguments = self._project_scoped_tool_arguments(name, self._parsed_tool_arguments(tc_data))
                    yield {"type": "tool_start", "name": name, "id": tc_data["id"], "arguments": arguments}
                    ...
                    if block_reason:
                        ...
                        if not task_board and on_first_tool:
                            task_board = _call_on_first_tool(on_first_tool, name, arguments)
                            if task_board:
                                rich = _emit_task_board_update(task_board, update="created", on_board_update=on_board_update, on_board_event=on_board_event)
                                yield {"type": "task_update", "board": rich}
                        result = self._dispatch_tool(name, arguments)
                        # Auto-advance only when the tool plausibly satisfies the active row.
                        # Final/report rows wait for the actual final answer.
                    ...
                    yield {"type": "tool_end", "name": name, "id": tc_data["id"], "result": result, "blocked": bool(block_reason)}
            ...
            if devmode05_active and total_tool_calls == 0 and not self._devmode05_taskboard_completed(task_board):
                ...
                yield {"type": "status", "message": "DEVMODE05: no tool evidence — continuing..."}
                continue
            ...
            if vs05_active and total_tool_calls == 0:
                ...
                yield {"type": "status", "message": "VS05: no tool evidence - continuing..."}
                continue

            if not vs05_final_allows_stop(user_input, content):
                self.session.add_assistant(vs05_continuation_instruction(user_input, content))
                yield {"type": "status", "message": "Continuing VS05..."}
                continue

            if not devmode05_final_allows_stop(user_input, content):
                self.session.add_assistant(devmode05_continuation_instruction(user_input, content))
                yield {"type": "status", "message": "Continuing DEVMODE05..."}
                continue
            ...
            if task_board:
                ...
                if rendered != getattr(self, '_last_rendered_board', None):
                    self._last_rendered_board = rendered
                    rich = _emit_task_board_update(task_board, update="completed" if task_board.open_count() == 0 else "updated", on_board_update=on_board_update, on_board_event=on_board_event)
                    yield {"type": "task_update", "board": rich}
            ...
            if self._self_protocol_completion_boundary_requires_continuation(user_input, final_text, boundary_report):
                self.session.add_assistant(self._self_protocol_task_truth_continuation_instruction(user_input))
                yield {"type": "status", "message": "Continuing self protocol: completion conflicts with open work..."}
                continue
            self.session.add_assistant(final_text, reasoning_content=reasoning)
            # Turn-end security check on modified files and response text
            if turn_modified_files:
                sec_result = run_turn_security_check(turn_modified_files, final_text)
                if sec_result.findings and monitor:
