...
- **The operator pack is a *profile-private pack* at `~/.mo/operator`, optionally Git-backed for its own backup — it is NOT a nested repo or submodule inside this product checkout.** There is exactly one product repo (`IQMO/MO`). The pack having its own `.git`/backup remote is just how that private profile data is backed up; the resolver (`core/path_defaults.py`) never treats a repo-local `operator/` as a source, and `tests/test_path_defaults.py` enforces it. Call it the "operator pack" or "private lane", never "the operator repo".
