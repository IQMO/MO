...
- **The operator pack is profile-owned operator state at `~/.mo/operator`; it is NOT a nested repo or submodule inside this product checkout.** There is exactly one product repo (`IQMO/MO`). The resolver (`core/path_defaults.py`) never treats a repo-local `operator/` as a source, and `tests/test_path_defaults.py` enforces it. Call it the "operator pack" or "profile-owned operator state", never a repo.
