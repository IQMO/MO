...
- **Multi-instance model:** multiple terminal MO instances are allowed. Each process gets a stable `MO_INSTANCE_ID` and its own default session slot (`main-<instance>`), unless `runtime.shared_session: true` is explicitly set for legacy shared-main behavior. Singleton resources (headless service, Telegram poller, scheduler, Desktop Companion tray/hotkey) are resource-locked, not blocked by every MO terminal.
- **Self-maintenance run manifest:** an owner-only self-maintenance session directory carries a runtime-owned `manifest.json` — one authoritative projection of that run's outputs (monitor, economy, taskboard, artifacts, status), built only from runtime truth, never model-authored. Any model-authored report in such a run must cite or be reconciled against it rather than restating independent counts.
