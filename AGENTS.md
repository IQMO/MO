...
- **Naming (partly migrated):** the desktop surface now *presents* as **Ghost** (window/tray/persona, `route_source`, config key `ghost`). Still transitional until the entity/process merge: the `/companion` command name and the `interface/companion` module path. End state unifies everything under **Ghost** with `/ghost` as the single command. See `docs/proposals/multi-instance-and-companion-ghost-separation.md`.
