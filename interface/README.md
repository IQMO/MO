...
- Scroll/selection contract: the styled prompt-toolkit TUI is the default when
  available. It keeps `full_screen=False`, `mouse_support=False`, and uses
  keyboard transcript scrolling (Up/Down, PageUp/PageDown). Plain native
  scrollback is an explicit fallback via `MO_NATIVE_SCROLL=1`.
