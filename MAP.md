...
- `core/learning/` — episodic memory (FTS5 + optional local-embedding RRF recall, env-tunable horizon `MO_MEMORY_MAX_TURNS`), workflow/feedback/trace learning. Per-turn mining stays inert, but the narrow universal safe class auto-promotes (`learning.auto_promote`, default on) so confirmed learnings actually inject; everything else needs `/learning confirm`. `/learning reconcile` consolidates confirmed near-duplicates (deterministic); `/learning reconcile deep` runs a propose-only dialectic operator-model pass (`core/learning/profile_reconcile.py`, one cheap ghost call, never rewrites profile prose). Operator corrections record the skill `correction` outcome (un-blinding retirement) and log to the inert `skill_evolution.json`. Transient session-health canaries (context-pressure / provider-error / no-memory-index diagnostics) are session telemetry, not durable learnings, so they are dropped at the suggestion-write boundary and never crowd the store.
