...
- Use Python for all scripting. Tests run with `python -m pytest -q`; full suite in parallel with `-n auto --dist loadfile` (~1 min). See MAP.md for the auto-tiered `-m smoke` / `-m "smoke or unit"` sweeps.
